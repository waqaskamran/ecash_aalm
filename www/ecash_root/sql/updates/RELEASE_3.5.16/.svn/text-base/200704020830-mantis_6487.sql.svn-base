-- Reload the application update trigger with work phone and extension audit.

-- This change also changes everything applicatable to use NOT (value1 <=> value2) instead of
-- checking for regular not equal or only one null. Saves lines and makes it easier to read.

DROP TRIGGER `app_updt`;
