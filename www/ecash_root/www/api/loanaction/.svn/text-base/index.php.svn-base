...
if (array_key_exists('wsdl', $_GET))
{
	...
	if ($_GET['wsdl'] == 'LoanActionHistoryService.wsdl') $name = $_GET['wsdl'];
	else $name = 'action.wsdl';
	$tokens = array('%%%soap_url%%%' => htmlentities($soap_url));
...
if ($security->loginUser('eCash_RPC', $username, $password))
{
	...
	else
	{
		...
		throw new SoapFault(ECash_Service_LoanActionHistoryService_Exception::CREDENTIALS_API_ACCESS,
...
else
{
	header('HTTP/1.0 401 You do not have access to this API');
	header('WWW-Authenticate: Basic realm="ECash"');
	throw new SoapFault(ECash_Service_LoanActionHistoryService_Exception::CREDENTIALS_LOGIN,
...
$api_class = ECash::getFactory()->getClassString("LoanActionHistoryService_API");
