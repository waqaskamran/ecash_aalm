...
/**
 * ECash Commercial Inquiry Sevice API
 *
...
if (array_key_exists('wsdl', $_GET))
{
	...
	$wsdl = file_get_contents(ECASH_SHARED_DIR.'/ECash/Service/aggregate.wsdl');
...
if ($security->loginUser('eCash_RPC', $username, $password))
{
	...
	else
	{
		...
		throw new SoapFault(ECash_Service_AggregateService_Exception::CREDENTIALS_API_ACCESS,
...
else
{
	header('HTTP/1.0 401 You do not have access to this API');
	header('WWW-Authenticate: Basic realm="ECash"');
	throw new SoapFault(ECash_Service_AggregateService_Exception::CREDENTIALS_LOGIN,
						"Invalid Username and/or Password.");
}

$api_factory = new ECash_AggregateService_ECashAPIFactory($failover_db, $company);
...
$api_class = ECash::getFactory()->getClassString("AggregateService_API");
