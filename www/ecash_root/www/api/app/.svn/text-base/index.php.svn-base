...
/**
 * ECash Commercial Application Sevice API
 *
...
if (array_key_exists('wsdl', $_GET))
{
	...
	$wsdl = file_get_contents(ECASH_SHARED_DIR.'/ECash/Service/app.wsdl');
...
if ($security->loginUser('eCash_RPC', $username, $password))
{
	...
	else
	{
		...
		throw new SoapFault(ECash_Service_ApplicationService_Exception::CREDENTIALS_API_ACCESS,
...
else
{
	header('HTTP/1.0 401 You do not have access to this API');
	header('WWW-Authenticate: Basic realm="ECash"');
	throw new SoapFault(ECash_Service_ApplicationService_Exception::CREDENTIALS_LOGIN,
...
$soap_server = new SoapServer($soap_url , array('cache_wsdl' => WSDL_CACHE_NONE));

$api_class = ECash::getFactory()->getClassString("ApplicationService_API");
