...
class HMS_Fix_Supended_batch
{
	...
	public function run()
	{
		...
		foreach($sorted_data as $company => $company_data)
		{
			...
			foreach($company_data['credit'] as $credit_row)
			{
				...
				$row = array();
				$row['app_id'] = $credit_row->application_id;
				$row['lender_tag'] = $credit_row->lender_tag;
				$row['last_name'] = $credit_row->last_name;
				$row['first_name'] = $credit_row->first_name;
				$row['payment'] = $credit_row->total_payment/100;
				$row['principal'] = $credit_row->principal/100;
				$row['service_charge'] = $credit_row->service_charge/100;
				$row['event_name'] = $reverse_map[$credit_row->transaction_type];
				$out_file[] = $row;
			...
			{
				$row = array();
				$row['app_id'] = $debit_row->application_id;
				$row['lender_tag'] = $debit_row->lender_tag;
				$row['last_name'] = $debit_row->last_name;
				$row['first_name'] = $debit_row->first_name;
				$row['payment'] = $debit_row->total_payment/100;
				$row['principal'] = $debit_row->principal/100;
				$row['service_charge'] = $debit_row->service_charge/100;
				$row['event_name'] = $reverse_map[$debit_row->transaction_type];
				$out_file[] = $row;

			}
		}
		$this->writeCSV($out_file, 'temp.csv');
