...
class Report extends Report_Generic
{
	...
	public function Generate_Report()
	{
		try
		{
			$this->search_query = new DNL_Report_Query($this->server);
			...
			$_SESSION['reports']['dnl']['report_data'] = new stdClass();
			$_SESSION['reports']['dnl']['report_data']->search_criteria = $data->search_criteria;
			$_SESSION['reports']['dnl']['url_data'] = array('name' => 'DNL Report', 'link' => '/?module=reporting&mode=dnl');
			...
			$data->search_results = $this->search_query->Fetch_DNL_Data($start_date_YYYYMMDD,
										  	$end_date_YYYYMMDD,
		...
		ECash::getTransport()->Set_Data($data);
		$_SESSION['reports']['dnl']['report_data'] = $data;
	}
}

class DNL_Report_Query extends Base_Report_Query
{
	private static $TIMER_NAME    = "DNL Report Query";
	...
	public function Fetch_DNL_Data($date_start, $date_end, $company_id, $loan_type)
	{
	...
	UPPER(com1.name_short) as company_owner,
	if(ag.name_last is null, 'Unknown', concat(lower(ag.name_first), ' ', lower(ag.name_last))) as agent_name

				from
	do_not_loan_flag as dnl

					join company as com on (dnl.company_id = com.company_id)
					join do_not_loan_flag_category as cat on (dnl.category_id = cat.category_id)
					join application as app using (ssn)
					...
					where
	dnl.active_status = 'active' AND
	dnl.date_created BETWEEN '{$timestamp_start}' AND '{$timestamp_end}'
					and
	dnl.company_id IN ({$company_list})
					{$loan_type_sql}
