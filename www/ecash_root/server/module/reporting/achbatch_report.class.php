...
class Customer_ACH_Batch_Report_Query extends Base_Report_Query
{
	...
	public function Fetch_ACH_Data($date_start, $date_end, $loan_type, $company_id, $batch_type, $ach_batch_company)
	{
		...
		if ((!isset($loan_type)) || ($loan_type == 'all'))
			$loan_type_sql = "";
		else
		...
		$query = "
			SELECT	a.transaction_date AS report_date,
				a.batch_company_name AS batch_company_name,
			...
			FROM ( -- Debits made during date range
			    SELECT
				ab.ach_provider_id AS ach_provider_id, -- asm 81
				ach.ach_type AS ach_type, -- asm 81
				...
				tr1.clearing_type as clearing_type,
				ap.name as batch_company_name
			    FROM ach
				...
				JOIN ach_provider ap ON (ap.ach_provider_id = ab.ach_provider_id)				
			    JOIN
			    (
				SELECT tr.ach_id,
				       es.context,
			    ...
			    GROUP BY ab.ach_batch_id
			) AS a
			LEFT JOIN
			( -- Returns received during date range
			    SELECT
				returns.ach_provider_id AS ach_provider_id, -- asm 81
				returns.ach_type AS ach_type, -- asm 81
				...
				SUM(IF(returns.context != 'arrangement' AND returns.amount < 0 AND returns.ach_type = 'debit', 1, 0)) AS debit_num_returned, -- asm 81
				SUM(IF(returns.context != 'arrangement' AND returns.amount < 0 AND returns.ach_type = 'debit', ABS(returns.amount), 0)) AS debit_amount_returned, -- asm 81
				SUM(IF(returns.context != 'arrangement' AND returns.amount > 0 AND returns.ach_type = 'credit', 1, 0)) AS credit_num_returned, -- asm 81
				SUM(IF(returns.context != 'arrangement' AND returns.amount > 0 AND returns.ach_type = 'credit', ABS(returns.amount), 0)) AS credit_amount_returned, -- asm 81
				...
				FROM transaction_history AS th
					...
					JOIN ach ON (ach.ach_id = tr.ach_id) -- asm 81
					JOIN ach_batch AS ab ON (ab.ach_batch_id = ach.ach_batch_id) -- asm 81
				...
				GROUP BY return_date, ach_provider_id, ach_type -- asm 81
			) AS b ON (a.transaction_date = b.return_date AND a.ach_provider_id = b.ach_provider_id AND a.ach_type = b.ach_type) -- asm 81
			LEFT JOIN
			(   -- Debits made during date range that were eventually returned
			    SELECT
				debits.ach_provider_id AS ach_provider_id, -- asm 81
				debits.ach_type AS ach_type, -- asm 81
				...
				SUM(IF(debits.context != 'arrangement' AND debits.amount < 0 AND debits.ach_type = 'debit', 1, 0)) AS num_returned_debit_adj_day, -- asm 81
				SUM(IF(debits.context != 'arrangement' AND debits.amount < 0 AND debits.ach_type = 'debit', ABS(debits.amount), 0)) AS amount_returned_debit_adj_day, -- asm 81
				SUM(IF(debits.context != 'arrangement' AND debits.amount > 0 AND debits.ach_type = 'credit', 1, 0)) AS num_returned_credit_adj_day, -- asm 81
				SUM(IF(debits.context != 'arrangement' AND debits.amount > 0 AND debits.ach_type = 'credit', ABS(debits.amount), 0)) AS amount_returned_credit_adj_day, -- asm 81
				...
				GROUP BY transaction_date, ach_provider_id, ach_type -- asm 81
			) AS c ON (a.transaction_date = c.transaction_date  AND a.ach_provider_id = c.ach_provider_id AND a.ach_type = c.ach_type) -- asm 81";
		...
		$query = "
		SELECT	a.transaction_date AS report_date,
				'Authorize.net' AS batch_company_name,
				STR_TO_DATE(a.transaction_date,'%m/%d/%Y') AS grouping_date,
				a.num_debit_attempted AS debit_num_attempted,
				a.debit_amount AS debit_total_attempted,
				a.num_credit_attempted AS credit_num_attempted,
		...
		foreach ($data_ach as $ach_row)
		{
			...
			while(($card_idx < count($data_card)) && !$ach_next){
				if (strtotime($data_card[$card_idx]['report_date']) < strtotime($ach_row['report_date'])){
