...
class Report extends Report_Generic
{
	...
	public function Generate_Report()
	{
		...
		try
		{
			$this->search_query = new Charge_Off_Report_Query($this->server);
			...
			$_SESSION['reports']['charge_off']['report_data'] = new stdClass();
			$_SESSION['reports']['charge_off']['report_data']->search_criteria = $data->search_criteria;
			$_SESSION['reports']['charge_off']['url_data'] = array('name' => 'Charge-Off', 'link' => '/?module=reporting&mode=charge_off');
			...
			$data->search_results = $this->search_query->Fetch_Charge_Off_Data( 
					$start_date_YYYYMMDD,
					$end_date_YYYYMMDD,
		...
		ECash::getTransport()->Set_Data($data);
		$_SESSION['reports']['charge_off']['report_data'] = $data;
	}
}

class Charge_Off_Report_Query extends Base_Report_Query
{
	private static $TIMER_NAME    = "Charge-Off Report Query";
	 ...
	 * @returns array
	 */
	public function Fetch_Charge_Off_Data($start_date, $end_date, $loan_type, $company_id)
	{
		...
		$query = "-- eCash 3.0, File: " . __FILE__ . ", Method: " . __METHOD__ . ", Line: " . __LINE__ . "
			SELECT
				...
				(
					...
					ORDER BY
						ish.status_history_id DESC
					LIMIT 1
				)                                AS chargeoff_date,
				app.application_id               AS application_id,
				ass.name                         AS loan_status,
			...
			AND
				ass.name IN ('Second Tier (Sent)', 'Inactive (Recovered)', 'Inactive (Settled)', 'Bankruptcy Verified', 'Write-Off', 'Deceased Verified')
			HAVING
				( chargeoff_date IS NOT NULL AND chargeoff_date BETWEEN '{$start_date}' AND '{$end_date}' ) 
		";
