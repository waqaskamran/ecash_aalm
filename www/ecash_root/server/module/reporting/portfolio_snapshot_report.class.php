...
class Report extends Report_Generic
{
	...
	public function __construct(Server $server, $request, $module_name, $report_name)
	{
		...
		$this->search_query = new Portfolio_Snapshot_Report_Query($this->server);		
	}
	...
	public function Generate_Report()
	{
		try
		{
			...
			$_SESSION['reports']['portfolio_snapshot']['report_data'] = new stdClass();
			$_SESSION['reports']['portfolio_snapshot']['report_data']->search_criteria = $data->search_criteria;
			$_SESSION['reports']['portfolio_snapshot']['url_data'] = array('name' => 'Portfolio Snapshot Report', 'link' => '/?module=reporting&mode=portfolio_snapshot');
			...
			$_SESSION['reports']['portfolio_snapshot']['report_data']->search_criteria['specific_date'] = $specific_date =
							$data->search_criteria['specific_date_YYYY'] . '-' .
			                $data->search_criteria['specific_date_MM'] . '-' .
			                $data->search_criteria['specific_date_DD'];

			if($this->search_query->Check_Portfolio_Snapshot_Data($specific_date) == 'TRUE')
			{
		...
		//$_SESSION['reports']['portfolio_snapshot']['report_data'] = $data;
	}
	...
	public function Download_Report()
	{
		...
		{		
			try
			{
				$data->search_results = $this->search_query->Fetch_Portfolio_Snapshot_Data($_SESSION['reports'][$this->report_name]['report_data']->search_criteria['specific_date']);
			}
			catch (Exception $e)
			{
				$data->search_message = "Unable to execute report. Reporting server may be unavailable.";
...
class Portfolio_Snapshot_Report_Query extends Base_Report_Query
{
	private static $TIMER_NAME    = "Portfolio Snapshot Report Query";

	public function Fetch_Portfolio_Snapshot_Data($date)
	{
		...
		from resolve_portfolio_snapshot_report
		where report_date = ?
		";
	...
	public function Check_Portfolio_Snapshot_Data($date)
	{
		$query = "
		select
			'TRUE'
