...
class Report extends Report_Generic
{
	...
	public function Generate_Report()
	{
		try
		{
                        $this->search_query = new Agent_Comments_Report_Query($this->server);
			...
			$_SESSION['reports']['agent_comments']['report_data'] = new stdClass();
			$_SESSION['reports']['agent_comments']['report_data']->search_criteria = $data->search_criteria;
			$_SESSION['reports']['agent_comments']['url_data'] = array('name' => 'Agent Comments', 'link' => '/?module=reporting&mode=agent_comments');
			...
			$data->search_results = $this->search_query->Fetch_Agent_Comments_Data($start_date_YYYYMMDD,
											  $end_date_YYYYMMDD,
		...
		ECash::getTransport()->Set_Data($data);
		$_SESSION['reports']['agent_comments']['report_data'] = $data;
	}
}


class Agent_Comments_Report_Query extends Base_Report_Query
{
	private static $TIMER_NAME = "Agent Comments Report Query";
	...
	public function Fetch_Agent_Comments_Data($start_date, $end_date, $comment_flag, $company_id)
	{
		...
		if ($comment_flag == "1")
			$comment_flag_query = " AND com.is_resolved = 1 ";
		else if ($comment_flag == "0")
			$comment_flag_query = " AND com.is_resolved = 0 ";
		else
			$comment_flag_query = "";
		...
		$fetch_query = "
				...
				SELECT
					...
					com.date_created AS 'created_on',
					com.type,
					IF(com.is_resolved = 1,'Resolved','Unresolved') AS 'comment_flag',
					com.comment
				FROM
					application AS app
				JOIN
				...
				JOIN
					comment AS com ON (com.company_id IN ({$company_list})
								AND com.application_id = app.application_id)
				LEFT JOIN
				agent AS ag ON (ag.agent_id = com.agent_id)
				WHERE
					app.company_id IN ({$company_list})
                                {$comment_flag_query}
				AND
					com.date_created BETWEEN {$start_date} AND {$end_date}
				ORDER BY
					app.application_status_id
		";
