...
//         		
			...
			case "reattempt":
				$this->loan_servicing = $this->loan_servicing ? $this->loan_servicing : new Loan_Servicing($this->server,$this->request,$request->mode, $this);
				$this->loan_servicing->scheduleReattempt();
				ECash::getTransport()->Add_Levels('overview', 'schedule','view');
				break;
				...
				if (isset($this->request->document_list) && (count($this->request->document_list) > 0))
				{
					...
					if ($submit_found)
					{
						if(!empty($document_list)) 
						{
							...
							//log
							$log_doc = get_log("documents");
							$document_list_string = implode(", ", $this->request->document_list);
							$submit = $this->request->submit;
							$agent_id = ECash::getAgent()->getAgentId();
							$log_doc->Write("Agent ID: " . $agent_id . ", " . $submit . ", App ID: " . $application_id . ", Doc List: " . $document_list_string);
						} 
						else 
						{
			...
			case "upload_document":
				...
				if ($_FILES["upload_document"]["size"] > 2100000) {
				...
				if ($uploadOk) {
					if ($this->request->submit == 'Upload Document') {
						...
						$type = 'text/html';
						$sender = 'Postal Mail';
						$recipient = 'customerservice@bluehorizonloans.com';
						$subject = 'Postal Mail Document';
						$data = 'See the attached PDF file';
						$condor = eCash_Document_DeliveryAPI_Condor::Prpc();
						if(move_uploaded_file($_FILES['upload_document']['tmp_name'],"/uploads/upload.pdf"))
						{
							if($archive_id = $condor->Incoming_Document('UPLOAD',$sender,$recipient,$_FILES["upload_document"]["type"],file_get_contents("/uploads/upload.pdf"),1,$_FILES["upload_document"]["name"]))
							{
								if($document = $documents->getByIDFromCondor($archive_id, $this->request->docname_other, $document_name))
								{
									$document->recieved('upload', ECash::getAgent()->getAgentId(), false);
								}
								else
								{
									$_SESSION['error_message'] = 'Document - Archive ID '.$archive_id.' Could Not Load Parent Doc to Set as Received.';
								}
							}
							else
							{
						...
						{
							$_SESSION['error_message'] = 'Document Failed to Upload.  Error Code: '.$_FILES['upload_document']['error'];
						}
					}
				}
