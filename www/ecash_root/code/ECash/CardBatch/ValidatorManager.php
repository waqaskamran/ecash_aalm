<?php
class ECash_CardBatch_ValidatorManager
{
	/**
	...
	public function __construct()
	{
		$this->tests = array();
		$this->messages = array();
		$this->loadTests(ECASH_CODE_DIR . 'ECash/CardBatch/ValidatorTest/', "ECash_CardBatch_ValidatorTest_");
	}	
			...
			foreach($files as $file) {
				...
				if( !($key = array_search($file, $tests))) {
					...
					try{	
						...
						if($class instanceof ECash_CardBatch_IValidator)
						{
							$tests[] = $class;
						}
						else
						{
							ECash::getlog('ach')->write("validator test $class_name is not instance of ECash_CardBatch_Validator");
						}
					}
					catch(Execption $e)
					{
