...
*
*/
class ECash_ACHReport_EPP_Returns extends ECash_ACHReport_Returns
{
	public function __construct(ECash_ACHReport_EPP_IReturnConfig $config)
	{
	...
	public function run()
	{
		...
		//Update Status.
		//Get File
		$filename = $config->getEPPReturnsFilename();
		if($this->getProcessState() == 'completed')
		{
			return true;
		}
		$this->setProcessState('started');
		if($file = $this->retrieveFile($config->getEPPReturnsTransport(),$filename))
		{
			//Store File
			$this->report_id = $this->storeFile($file,$filename,'returns');
			//Parse File
			$records = $this->parseFile($file, $config->getEPPReturnsParser());
			//Update Records
