<?php
	/**
	 * ECash Transport Object that will soon be removed
	 * but we have it here so we can help get rid of the Server Object
	 */
	class ECash_Transport extends Object_1
	{
		...
		public $pbx_enabled;
		public $acl;
		public $user_acl_names;
		public $user_acl_sub_access;
		public $user_acl_sub_names;
	...
	//		if (EXECUTION_MODE == 'LOCAL' && !is_object($data)) throw new Exception("Passing non-object data: " . var_export($data, true));
			...
			elseif (is_object($data) && is_array($this->data))
			{
				$this->data = array_merge($this->data, (array) $data);
			}
			elseif (is_array($data) && is_object($this->data))
			{
