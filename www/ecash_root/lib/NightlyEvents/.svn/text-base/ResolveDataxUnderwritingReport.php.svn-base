...
class ECash_NightlyEvent_ResolveDataxUnderwritingReport extends ECash_Nightly_Event
{
		// Parameters used by the Cron Scheduler
	protected $business_rule_name = 'resolve_datax_underwriting';
	protected $timer_name = 'Resolve_Datax_Underwriting';
	protected $process_log_name = 'resolve_datax_underwriting';
	...
	public function run()
	{
		...
		$this->Resolve_Datax_Underwriting_Report($this->start_date);
	}

	private function Resolve_Datax_Underwriting_Report($run_date)
	{
...
ORDER BY company, (if(t1.name_first LIKE '%test%' OR t1.name_last LIKE '%test%',1,0)),  status DESC, `Application ID` ASC
		...
		$insert = 'replace into resolve_datax_underwriting_report
				(report_date, result)
				values
				(?, COMPRESS(?))';
