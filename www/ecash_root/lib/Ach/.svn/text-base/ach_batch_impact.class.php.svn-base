 ...
 *
 */
class ACH_Batch_Impact extends ACH_Batch
{
// Record Structure for NACHA Deposit File Format
	...
	/**
	 * Overridden method for Impact/HMS since we have a few
	 * differences between the two file naming conventions.
	 *
	 * This should allow us to customize the filename for HMS
	 * without making any changes to Impact. [BR]
	 * 
	 * @return string
	 */
	public function Get_Remote_Filename()
	{
		// Per #22364, the filename should use the date in which
		// we sent the batch to the processor, not the ACH date.
		$file_date = date('Ymd');
		
		$transport_type   = ECash::getConfig()->ACH_TRANSPORT_TYPE;
		$transport_url    = ECash::getConfig()->ACH_BATCH_URL;
		$batch_url_prefix = ECash::getConfig()->ACH_BATCH_URL_PREFIX;
		$batch_url_suffix = ECash::getConfig()->ACH_BATCH_URL_SUFFIX;

		/**
		 * Format used by HMS, possibly others.  Allows us to specify any company name
		 * prefix we want and also whatever suffix we want.
		 */
		if(! empty($batch_url_prefix) && !empty($batch_url_suffix))
		{
			$filename = $batch_url_prefix . "_" . $this->batch_type ."_" . $file_date . $batch_url_suffix;
		}
		else
		{
