...
/**
 * LST Specific Batch Processing Implementation
 * 
 * LST uses a standard NACHA file format so this file is simply
 * a copy of the USBank processor class.  I would have used the 
 ...
 */
class ACH_Batch_LST extends ACH_Batch
{
	...
	protected static $achstruct =  array(
					      ...
					      array(	'record_type_code'				=> array( 1,  1, 'A', '1'),
							...
							'immediate_origin_name2'		=> array(87,  8, 'A') //specific to LST
							),
					      5 =>
	...
	protected function Build_ACH_File($ach_transaction_ary)
	{
		...
		foreach ($ach_transaction_ary AS $record)
		{
			...
			if(ECash::getConfig()->ACH_USE_APP_ID_FOR_NAME === TRUE)
			{
				...
				$output['individual_name']			= substr($record['full_name'], 0, 22);
			}
		...
		/**
		 * LST is a combined batch format so we're only
