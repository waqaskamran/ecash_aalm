...
/**
 * Class for handling returns for Advantage ACH / ACH Commerce.
 *
 * Loosely based on Impact's ACH returns processing code.  There's
 * nothing exceptional in this code, it's rather generic. [BR]
 *
 * History:
...
class ACH_Return_AdvantageACH extends ACH_Return
{
	...
	/**
	 * [#55816] the report format is now contained within the
	 * AGEAN_CSVParser - method remains here for the interface
	 */
	...
	public function Process_ACH_Report_Data ($response, $report_type)
	{
		...
		try
		{
			 ...
			 */
			if(! $ach_report_data = AGEAN_CSVParser::parse($response['received']))
			{
				return FALSE;
			}
			...
			{
				//[#54784] trim extra space, and restore recipient name
				foreach($report_data as &$raw_data)
				{
					$raw_data = trim($raw_data);
				}
				//[#55816] combine the names if they were previously in seperate fields
				if(isset($report_data['recipient_name_last']))
				{
					$report_data['recipient_name'] =  $report_data['recipient_name_last'] . ', ' . $report_data['recipient_name_first'];
				}
				...
				if (is_numeric($ach_id))
				{
					...
					elseif ($report_type == 'corrections')
					{
						...
						$corrected_data_ary = preg_split("/\s+/", $report_data['corrected_info']);
	...
	/**
	 * [#55816] the report format is now contained within the
	 * AGEAN_CSVParser - parm remains here for the interface
	 */
	public function Parse_Report_Batch ($return_file, $report_format)
	{
		try
		{	
			return AGEAN_CSVParser::parse($return_file);
		}
		catch(Exception $e)
		{
			return false;
