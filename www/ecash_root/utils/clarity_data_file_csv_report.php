...
/*
	...
	NEW FIELDS
	    paycheck_direct_deposit    (0 or 1 based on application.income_direct_deposit)
	    date_of_next_payday - comes from ? 
        occupation_type            (application.income_source)
	    net_monthly_income         (a.k.a. MonthlyIncome see: a.income_monthly)
	    months_at_current_employer (application.job_tenure) 
...
$fp = fopen("/tmp/clarity_data_{$date_start}-{$date_end}_run_{$date_today}.csv", 'w');
fputcsv($fp, $columns);
...
'n/a' as date_of_next_payday,
a.job_tenure as months_at_current_employer,
".
// a.paydate_model,
"a.application_id
from application a
