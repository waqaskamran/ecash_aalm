<?php
/**
php compare_aalm-ldb_version_data.php
*/
...
FROM application AS ap
    JOIN application_version AS av ON (ap.application_id = av.application_id)
...
$query_ldb = "
SELECT av.application_id, 
        av.version AS version
FROM application_version av
    ORDER BY av.application_id
";
...
foreach (array_keys($chasm_keys) as $key){
    if (!$null_test_array[$key]) {
        echo "\n  all of version fields [".$key."] are null in chasm.";
        $not_one_fail = true;
    }
    if (!$empty_test_array[$key]) {
        echo "\n  all of version fields [".$key."] are empty in chasm.";
        $not_one_fail = true;
    }
}
if ($not_one_fail) echo "\n   none found.";

echo "\n\nChasm fields match to LDB by version test:";
$i = 0;
$not_one_fail = true;
...
//                    ) &&
    ...
    } else $chasm_missing_array[] = "\n  ap: ".$ldb_row->application_id. '  version:'.$ldb_row->version;
}
if ($not_one_fail) echo "\n   none found.";

echo "\n\nMissing version test:";
echo "\n ldb version ids missing from chasm";
