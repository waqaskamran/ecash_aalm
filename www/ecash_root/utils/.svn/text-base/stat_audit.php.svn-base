...
<?php
	...
	else
	{
		...
		$enterprise = strtolower($enterprise_prefix);
	...
	class StatAuditReport
	{
		...
		//public function __construct($ecash_db, $mgmt_db, $statpro_cust, $statpro_pass, $statpro_mode, $pretend)
		public function __construct($ecash_db, $mgmt_db, $enterprise, $statpro_mode, $pretend)
		{
			...
			// statpro database
			$db = strtolower($statpro_cust).'_statpro_data';
			
			switch (strtolower($statpro_mode))
			{
				case 'live':
					$this->statpro_db = new MySQLi_1('reader.statpro.ept.tss', 'owner', 'llama', $db, 3307);
		...
		public function Prepare($start_date, $end_date, $companies = NULL)
		{
			...
			if (!$this->event_map) $this->getEventMap();
		...
		protected function queryApplications($start_date, $end_date, $companies = NULL)
		{
			...
			$query = "
				...
				WHERE
					...
					-- AND status_history.application_id = 44572255
		...
		protected function processBatch(Array $batch)
		{
			...
			{
				if($rec['track_key'] != $last_track_key)
				{
					$last_space     = NULL;
					$last_track_id  = NULL;
					$last_track_key = NULL;
					...
					if (($rec['track_id'] = $this->getTrackID($rec['track_key'])) === FALSE || empty($rec['track_id']))
					{
					...
					{
						$last_track_key = $rec['track_key'];
						$last_track_id  = $rec['track_id'];
						$last_space = NULL;
					}
				}
				else
				{
					$rec['track_id'] = $last_track_id;
				}
				...
				try
				{
					...
					// convert to IDs
					$event_ids = $this->doMap($this->event_map, $events);
		...
		protected function addMissing($space, $rec, $event_ids)
		{
			...
			$events = $this->doMap($this->event_id_map, $event_ids);
		...
		/**
		 * NOTE: No current references to this method!
		 */
		protected function getTrackIDs(Array $track_keys)
		{
			
			// escape 'em all! 
			$track_keys = array_map(array($this->statpro_db, 'Escape_String'), $track_keys);
			...
			$query = "
				...
				FROM
					track
				WHERE
					track_key IN ('".implode("', '", $track_keys)."')
			";
			...
			{
				$map[$rec['track_key']] = $rec['track_id'];
			}
		...
		protected function getTrackID($track_key)
		{
		...
		/**
		 * NOTE: Currently no references to this!
		 */
		protected function getSpaceDefinition($space_key)
		{
			...
			$query = "
				SELECT
					...
					promo_sub_code
				FROM
					enterprisepro.space_definition
				WHERE
					space_key = '".$this->statpro_db->Escape_String($space_key)."'
			";
			$result = $this->statpro_db->Query($query);
		...
		protected function checkTrack($track_id, $event_ids, $timestamp = NULL, $inverse = FALSE, $fudge = 300, $space_key = NULL)
		{
			...
			{
				$query .= "
					AND date_occured > ".($timestamp - $fudge)."
					AND date_occured < ".($timestamp + $fudge)."
				";
			}
			...
			while ($rec = $result->Fetch_Array_Row())
			{
				if (($key = array_search($rec['event_type_id'], $event_ids)) !== FALSE)
				{
					...
					//echo "Event '", $this->doMap($this->event_id_map, $rec['event_type_id']) ,"' was ", ($rec['date_occured'] - $timestamp),
					//	" seconds off; ", date('Y-m-d H:i:s', $timestamp), " ", date('Y-m-d H:i:s', $rec['date_occured']), "\n";
				}
			}
		...
		protected function getEventMap()
		{
			...
			while ($rec = $result->Fetch_Array_Row())
			{
				$this->event_map[$rec['event_type_key']] = $rec['event_type_id'];
				$this->event_id_map[$rec['event_type_id']] = $rec['event_type_key'];
			}
