...
foreach (array_keys($chasm_keys) as $key){
    ...
    if (!array_key_exists($key,$ldb_keys)) {
        echo "\n  missing chasm do_not_loan_flag field [".$key."] in ldb.";
...
foreach (array_keys($chasm_keys) as $key){
    if (!$null_test_array[$key]) {
        echo "\n  all of do_not_loan_flag fields [".$key."] are null in chasm.";
        $not_one_fail = true;
    }
    if (!$empty_test_array[$key]) {
        echo "\n  all of do_not_loan_flag fields [".$key."] are empty in chasm.";
        $not_one_fail = true;
    }
}
if ($not_one_fail) echo "\n   none found.";

echo "\n\nChasm fields match to LDB by do_not_loan_flag test:";
$i = 0;
$not_one_fail = true;
...
echo "\n\nMissing do_not_loan_flag test:";
echo "\n ldb do_not_loan_flag ids missing from chasm";
...
$i = 0;
echo "\n chasm do_not_loan_flag ids missing from ldb";
foreach ($rows_chasm as $chasm_row){
