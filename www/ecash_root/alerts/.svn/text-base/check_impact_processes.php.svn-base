...
try 
{
	...
	$check_processes = array(
			'ach_returns' 		=> array('time' 	=> '10:00 pm',
										 'period' 	=> 'after', 
										 'run_holidays' => false,
										 'run_weekends' => false,
										 'alternate_process' => null,
										 'notification_list' => NOTIFICATION_ERROR_RECIPIENTS,
										 'error_message' => 'The ACH Returns from ' . date('m/d/Y') . ' have not processed as of '. date('H:i:s m/d/Y') .'.',
										 'subject_label' => 'Impact: ACH Returns'),
// Corrections are ignored for Impact because they do not always recieve a corrections file
...
//										 'subject_label' => 'ACH Corrections'),
			'ach_reschedule' 	=> array('time' 	=> '10:00 pm',
										 'period' 	=> 'after', 
										 'run_holidays' => false,
										 'run_weekends' => false,
										 'alternate_process' => null,
										 ...
										 'subject_label' => 'Impact: ACH Rescheduling'),
			'nightly' 			=> array('time' 	=> '9:30 pm',
										 'period' 	=> 'after', 
										 'run_holidays' => true,
										 'run_weekends' => true,
										 'alternate_process' => null,
	...
	foreach($check_processes as $process => $parameters)
	{
		...
		if($run) {
			...
			$process_companies = $db->querySingleColumn($sql);
			...
			foreach ($eligible_companies as $e) {
				...
				{ 
					...
					$body = "eCash 3.0: The {$subject} Process may have failures for {$e->name}!"
						. "\n\n" . $error_message; 
					Email_Report($notification_list, $body, $subject);
					//echo $str . "\n";
					$HAVE_FAILURES = TRUE;
				}
			}
...
function Email_Report($recipients, $body, $subject = null) {
	require_once(LIB_DIR . '/Mail.class.php');
	return eCash_Mail::sendExceptionMessage($recipients, $body, $subject);
}
 ...
 */
function Get_Eligible_Companies() 
{
	$db = ECash::getMasterDb();
