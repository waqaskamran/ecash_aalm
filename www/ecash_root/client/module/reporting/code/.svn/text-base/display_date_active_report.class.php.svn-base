 ...
 * @category Display
 */
class Date_Active_Report extends Report_Parent
{
	public function __construct(ECash_Transport $transport, $module_name)
	{
		$this->report_title       = "Certegy Fulfilled";
		$this->column_names       = array( 'application_id'          => 'Application ID',
		                                   'date_application_status_set' => 'Date Active');
	
		$this->column_format	  = array( 'date_application_status_set' => self::FORMAT_DATE);
//		$this->column_width		  = array('description'		=>	250);
		$this->totals             = array('company'   => array('date_application_status_set' => Report_Parent::$TOTAL_AS_COUNT));
...
/**
	...
	protected function Get_Data_HTML($company_data, &$company_totals)
		...
		$this->totals['company']['dates'] = array();
				...
				if( $this->check_eval($company_data[$x], $data_name) && isset($this->totals['company'][$data_name]) )
				{
					switch($this->totals['company'][$data_name])
					{
						case self::$TOTAL_AS_COUNT:
							if($data_name == 'date_application_status_set')
							{
								
								$this->totals['company']['dates'][$data]++;
							}
							else 
							{
...
/**
	...
	protected function Get_Total_HTML($company_name, &$company_totals)
	{
			...
			(empty($this->totals['company']['rows']) && count($this->totals['company']) > 0 ))
		{
			...
			foreach( $this->column_names as $data_name => $column_name )
			{
				if( ! empty($this->totals['company'][$data_name]) && $data_name != 'date_application_status_set')
				{
					$align = 'left';
			...
			$line .= "    </tr>\n";
			$total = 0;
			foreach ($this->totals['company']['dates'] as $date => $count) {
				$align = 'left';
				$line .= "     <tr><th class=\"report_foot\" style=\"text-align: $align;\">$date</th>\n";
				$align = 'right';
                    ...
                    switch($this->totals['company'][$data_col_name])
                    {
                        case self::$TOTAL_AS_COUNT:
                            if($data_name == 'date_application_status_set')
							{
								...
								$this->totals['company']['dates'][ $this->Format_Field($data_col_name, $company_data[$x][$data_col_name], false, false)]++;
							}
							else 
							{
								$company_totals[$data_name]++;
