...
class Payments_Due_Report extends Report_Parent
{
	...
	*/
	...
	{
		$this->report_title = "Projected Payments Due";
	...
	public function Download_Data()
	{
		...
		if( $this->loan_type === true )
		{
			switch($this->search_criteria['loan_type'])
			{
				...
				default:
					//GForge [#25926]
					//not sure if the above cases are needed anymore
					$type_not_found = TRUE;
					foreach($this->loan_type_list as $loan_type)
					{
						if($type_not_found && $this->search_criteria['loan_type'] == $loan_type['name_short'])
						{
							$type_not_found = FALSE;
							$dl_data .= "For loan type: {$loan_type['name']}\n";
						}
					}
					if($type_not_found)
