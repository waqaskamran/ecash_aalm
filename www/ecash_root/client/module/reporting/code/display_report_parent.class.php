	...
	*/
	abstract class Report_Parent implements Display_Module {
		...
		*/
		...
		//asm 80
		/**
		* Should the batch_type (ach/card) selection box be displayed?
		* @var	boolean
		* @access protected
		*/
		...
		/**
		* Should the ach batch provider selection box be displayed?
		* @var	boolean
		* @access protected
		*/
		protected $ach_batch_company;
		protected $ach_providers;
		...
		*/
		...
		/**
		 * Should the date fund actual be displayed?
		 * @var	boolean
		 * @access protected
		 */
		protected $date_fund_range;
		...
		public function __construct(ECash_Transport $transport, $module_name) {
			...
			if(!isset($this->totals)) $this->totals = array( 'company' => array(), 'grand' => array() );
			...
			//asm 80
			if( !isset($this->batch_type) || ! is_bool($this->batch_type) )
				$this->batch_type = false;
			if( !isset($this->ach_batch_company) || ! is_bool($this->ach_batch_company) )
				$this->ach_batch_company = false;
			////////
...
//error_log(print_r($columns,true));			
			...
			$this->ach_providers = ( isset($temp->ach_providers) ? $temp->ach_providers : null ); //asm 80
...
//					Never before funding date
			...
			if( $this->loan_type === true ) {
				...
				$substitutions->loan_type_select_list = '<span>'. $this->product_type .' Type : </span><span><select name="loan_type" size="1" style="width:70px;">';
			...
			if( $this->collection_level === true ) {
				$substitutions->loan_type_select_list = '<span>Collection Level: </span><span><select name="collection_level" size="1" style="width:auto;"></span>';
			...
			//asm 80
			if( $this->batch_type === true ) {
				$substitutions->loan_type_select_list = '<span>Batch Type: </span><span><select name="batch_type" size="1" style="width:50px;"></span>';
				switch( $this->search_criteria['batch_type']) {
					case '':
						$substitutions->loan_type_select_list .= '<option value="" selected>All</option>';
						$substitutions->loan_type_select_list .= '<option value="ach">Ach</option>';
						$substitutions->loan_type_select_list .= '<option value="card">Card</option>';
					break;
				
					case 'ach':
						$substitutions->loan_type_select_list .= '<option value="">All</option>';
						$substitutions->loan_type_select_list .= '<option value="ach" selected>Ach</option>';
						$substitutions->loan_type_select_list .= '<option value="card">Card</option>';
					break;
					...
					case 'card':
						$substitutions->loan_type_select_list .= '<option value="">All</option>';
						$substitutions->loan_type_select_list .= '<option value="ach">Ach</option>';
						$substitutions->loan_type_select_list .= '<option value="card" selected>Card</option>';
					break;
			...
			if( $this->ach_batch_company === true )
			{
				$substitutions->achtype_select_list = $this->getACHProvidersDropdown();
			}
			////////
			...
			if( $this->date_fund_range === true) {
				// Label
				$substitutions->extra_search_table_row = '<tr><td valign="top" nowrap="nowrap" align="left"><span>Fund Date:</span></td>';

				// Date Selector
				$substitutions->extra_search_table_row .= '<td valign="top" align="left">';
				
				$extra_attribute = 'onClick="ReportCalendar(\'date_range_\', event.clientX, event.clientY)"';
				$substitutions->extra_search_table_row .= '<span style="white-space: nowrap;">' . $this->Date_Calander( "date_range_", "range", $extra_attribute )
						. ' (<a style="text-decoration: underline;" href="#" ' . $extra_attribute . '>select</a>)</span></td>'.
						'<td/><td style="vertical-align: top; text-align: left;"><select name="date_fund_range_direction" size="1">';
										// Direction (GT or LT)
				switch( $this->search_criteria['date_fund_range_direction'] ) {
					case "lt":
						$substitutions->extra_search_table_row .= '<option value="all" >All Loans</option>';
						$substitutions->extra_search_table_row .= '<option value="gt">Greater Than</option>';
						$substitutions->extra_search_table_row .= '<option value="lt" selected>Less Than</option>';
					break;
					case "gt":
						$substitutions->extra_search_table_row .= '<option value="all" >All Loans</option>';
						$substitutions->extra_search_table_row .= '<option value="gt" selected>Greater Than</option>';
						$substitutions->extra_search_table_row .= '<option value="lt">Less Than</option>';
					break;
					case "all":
					default:
						$substitutions->extra_search_table_row .= '<option value="all" selected>All Loans</option>';
						$substitutions->extra_search_table_row .= '<option value="gt">Greater Than</option>';
						$substitutions->extra_search_table_row .= '<option value="lt">Less Than</option>';
					break;
				}
				$substitutions->extra_search_table_row .= '</select></td><td/><td/><td/></tr>';
			}
...
//	  tf.style.paddingRight = '18px;'
		...
		//asm 80
		protected function getACHProvidersDropdown()
		{
			$provider_array = array();

			if($this->search_criteria['ach_batch_company'] != NULL)
			{
				$provider_array[$this->search_criteria['ach_batch_company']] = 'SELECTED';
			}

			$list = "<span>Ach Provider: </span><span><select name='ach_batch_company' size='1' style='width:80px;'></span>";
			$list .= "<option value='' selected>All</option>";

			foreach($this->ach_providers as $key => $name)
			{
				$list .= "<option value='" . $key . "' " . $provider_array[$key] . ">" . $name . "</option>\n";
			}
			$list .= "</select>\n";
