...
class Display_View extends Display_Parent
{
	...
	public function Get_Module_HTML()
	{
		...
		{
			case  'external_apps':
				$this->data->second_tier_batches  = $this->Format_Batches();
				...
				$html = file_get_contents(CLIENT_MODULE_DIR . "{$this->module_name}/view/external_collections.html");
				break;
		}
	...
	private function Format_Batches()
	{
		...
		if (is_array($this->data->batches))
		{
			foreach ($this->data->batches as $batch)
			{
				...
				$download_summary = "<a href ='/?module=collections&mode=external&action=download_external_batch&report_id={$batch['batch_id']}&report_type=second_tier_batch'>download</a>";
				...
				$output .= "  <td class=\"it_settlement\" style=\"text-align: center;\">{$batch['batch_report']}</td>\n";
				$output .= "  <td class=\"it_settlement\" style=\"text-align: center;\">2nd Tier</td>\n";
				$output .= "  <td class=\"it_settlement\" style=\"text-align: center;\">{$batch['batch_num_apps']}</td>\n";
				$output .= "  <td class=\"it_settlement\" style=\"text-align: center;\">{$batch['batch_status']}</td>\n";
				$output .= "  <td class=\"it_settlement\" style=\"text-align: center;\">{$download_summary}\n";
				$output .= "</tr>\n";
